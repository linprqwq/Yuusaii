package com.lin.dao.impl;

import com.lin.dao.ZwDao;
import com.lin.pojo.Zw;
import com.lin.utils.DBUtil;
import org.apache.commons.dbutils.QueryRunner;
import org.apache.commons.dbutils.handlers.BeanHandler;
import org.apache.commons.dbutils.handlers.BeanListHandler;
import org.apache.commons.dbutils.handlers.ScalarHandler;

import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

public class ZwDaoImpl implements ZwDao {
    public List<Zw> queryallzw() {
        List<Zw> list =new ArrayList<Zw>();

        String sql ="select * from zw";

        try {
            list=new QueryRunner().query(DBUtil.getConn(),
                    sql,
                    new BeanListHandler<Zw>(Zw.class));
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return list;
    }


    public ArrayList<Zw> selectservlet(Zw r, int pagenum, int pagesize) {



        ArrayList<Zw> list =new ArrayList<Zw>();
        ArrayList params=new ArrayList();


        String sql="SELECT * FROM zw WHERE 1=1";

        if(r.getName()!=null && r.getName()!="") {
            sql+="  and name like concat('%',?,'%')";
            params.add(r.getName());
        }

        //分页功能
        sql+=" LIMIT  ?,?";
        params.add((pagenum-1)*pagesize);
        params.add(pagesize);



        QueryRunner queryRunner = new QueryRunner();
        try {
            list = (ArrayList<Zw>) queryRunner.query(DBUtil.getConn(),sql,new BeanListHandler<Zw>(Zw.class),params.toArray());
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return list;
    }



    public int querynewstypecountbycond(Zw r){

        int num =0;
        Object[] params = new Object[]{};
        String sql="select count(*) as totalrecord from zw where 1=1";

        if(r.getName()!=null && r.getName()!="") {
            sql+="  and name like concat('%',?,'%')";
            params = new Object[]{r.getName()};
        }


        QueryRunner queryRunner = new QueryRunner();
        try {
            num =queryRunner.query(DBUtil.getConn(),sql,new ScalarHandler<Long>(),params).intValue();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return num;
    }


    public int add(Zw r) {
        int num = 0;
        //1.写sql
        String sql = "insert into zw values(null,?,?)";
        //2.参数
        Object[] params = new Object[]{r.getName(),r.getRemark()};

        //3.sql执行器
        QueryRunner queryRunner = new QueryRunner();

        //4.执行sql语句
        try {
            num = queryRunner.update(DBUtil.getConn(), sql, params);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return num;
    }

    public Zw selectByid(int id) {


        Zw r = null;
        //1.sql
        String sql = "SELECT * FROM zw where id=?";
        //2.参数
        Object[] params = new Object[]{id};
        //3.执行器
        QueryRunner queryRunner = new QueryRunner();
        //4.执行
        try {
            r = queryRunner.query(DBUtil.getConn(),
                    sql,
                    new BeanHandler<Zw>(Zw.class),
                    params);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return r;
    }


    public int xg(Zw r) {
        int num = 0;
        //1.写sql
        String sql = "UPDATE zw SET NAME=?,remark=? WHERE id=?";
        //2.参数
        Object[] params = new Object[]{r.getName(),r.getRemark(),r.getId()};

        //3.sql执行器
        QueryRunner queryRunner = new QueryRunner();

        //4.执行sql语句
        try {
            num = queryRunner.update(DBUtil.getConn(), sql, params);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return num;
    }

    public int del(int id){

        int num = 0;
        //1.写sql
        String sql = "DELETE  FROM zw WHERE id=?";
        //2.参数
        Object[] params = new Object[]{id};

        //3.sql执行器
        QueryRunner queryRunner = new QueryRunner();

        //4.执行sql语句
        try {
            num = queryRunner.update(DBUtil.getConn(), sql, params);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return num;
    }
}
